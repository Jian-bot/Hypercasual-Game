# Hypercasual-Game
**3D Hypercasual Game series containing two games.**
* 3D Color Switch (Jian)
  - Player can hit part of obstacles that has the same color as the ball, otherwise explodes. 
* Pong (Ryan)
  - Two-player air hockey game.
## MainScreen UI
**Direct Users to different game or quit game.**

<img width="837" alt="MainMenu_UI" src="https://github.com/Jian-bot/Hypercasual-Game/assets/76824957/3824aa78-b1da-4e28-b5be-cbf0275c4374">

## 3D Color Switch UI
**Right clicked to hold the ball during game**
* **QUICK START:** Start from level 0.
* **MAIN:** Back to MainScreen UI.
* **QUIT:** Quit game.
* **LEVELS:** Select your favorite level to start the game. 
<img width="837" alt="ColorSwitch_UI" src="https://github.com/Jian-bot/Hypercasual-Game/assets/76824957/a87faee8-3763-4b1e-aa82-f21239574232">

## Game Lost 
* **Restart:** Restart the current level of game.
* **Home:** Back to 3D Color Switch UI.
<img width="837" alt="Game-lost_UI" src="https://github.com/Jian-bot/Hypercasual-Game/assets/76824957/6efe3e7a-24cb-45e3-8071-f34bc4cce700">

## 3D Color switch Scenes & Assets
**6 levels of game play with variety of fun obstacles and challenges:**
* Different speed (player & spinning obstacles)
* Different obstacles combination
* The ball is assigned a different mesh at each level.
<img width="837" alt="Screenshot 2023-09-02 200417" src="https://github.com/Jian-bot/Hypercasual-Game/assets/76824957/780fa56c-48e6-43a4-b639-5ff4686d5302">

**Assets**
* Models created using Blender and imported into Uity.
<img width="837" alt="Screenshot 2023-09-02 200445" src="https://github.com/Jian-bot/Hypercasual-Game/assets/76824957/77aaf75a-c303-4cd2-84ce-00ec24dd2b8c">
<img width="837" alt="Screenshot 2023-09-02 200458" src="https://github.com/Jian-bot/Hypercasual-Game/assets/76824957/c7d2c955-14ba-4367-84bc-cc871fc81f39">
